...
export const requestInterceptor = async config => {
  try {
    config.headers.common.Authorization = `Bearer ${getTokens()}`;
...
export const checkForce = async request => {
  //getting user unique UID for force login
  let uuid_parse = JSON.parse(window.localStorage?.[`${host}_uuid`]);
  let ud = uuid_parse.access.uuid;
  //  getting user id
  let uuid = localStorage.getItem(`${host}_uid`);
  let ans;
  await axios.post(`${USERS_BASE_URL}/auth/checkForce`, { uuid }).then(res => {
    ans = res.data;
  });
  ...
  if (accessTokenExpires >= exp_ud) {
    tokenFlag = 'true';
  } else {
    tokenFlag = 'false';
  }
  if (active_user_id != ud) {
    signout(tokenFlag);
    unSetSessionCookies();
