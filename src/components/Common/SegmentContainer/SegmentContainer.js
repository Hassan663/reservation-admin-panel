...
import { Typography, Input, Checkbox, Skeleton } from 'antd';
...
const SegmentContainer = ({
  ...
  const handleImageChange = ({ fileList }) => {
    ...
    dispatch(markerEditActions.updateSegmentField({ field: 'thumbnail', index, value: fileList }));
  };
  return (
          ...
          {isMarker ? (
            <>
              {isClipper && pointerevent !== 'none' ? (
                <PicturesWall handleImageChange={handleImageChange} fileList={thumbnail} />
              ) : (
                isClipper &&
                pointerevent == 'none' && (
